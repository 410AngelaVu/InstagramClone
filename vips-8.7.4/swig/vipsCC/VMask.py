...
if _swig_python_version_info >= (2, 7, 0):
    def swig_import_helper():
        ...
        mname = '.'.join((pkg, 'vmaskmodule')).lstrip('.')
        try:
            return importlib.import_module(mname)
        except ImportError:
            return importlib.import_module('vmaskmodule')
...
class VIMask(VMask):
    ...
    if _newclass:
        ...
        gauss = vmaskmodule.VIMask_gauss
    if _newclass:
        gauss_sep = staticmethod(vmaskmodule.VIMask_gauss_sep)
    else:
        gauss_sep = vmaskmodule.VIMask_gauss_sep
    if _newclass:
        log = staticmethod(vmaskmodule.VIMask_log)
    else:
        log = vmaskmodule.VIMask_log
...
class VDMask(VMask):
    ...
    if _newclass:
        ...
        gauss = vmaskmodule.VDMask_gauss
    if _newclass:
        log = staticmethod(vmaskmodule.VDMask_log)
    else:
        log = vmaskmodule.VDMask_log
